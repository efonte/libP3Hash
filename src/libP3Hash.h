...
namespace patapon {
    ...
    // in C++20 can be made constexpr

    const std::vector<uint32_t> magicTable = {
    ...
    private:
        ...
        void ccc();
        void ddd();
        static std::pair<uint32_t, uint32_t> decrypt_two_values(uint32_t var1, uint32_t var2);
    ...
    public:
        std::vector<char> encryptRawData(std::vector<char> data);
        std::vector<char> decryptRawData(std::vector<char> data);
